...
const Checkout = () => {
  ...
  const { triggerBurstConfetti } = useAnimation();
  ...
  // Redirect if cart is empty - use useEffect to avoid state updates during render
  useEffect(() => {
    if (items.length === 0) {
      navigate('/cart');
    }
        ...
        <div>
          <div className="bg-card border rounded-lg p-6 sticky top-24">
            ...
            <div className="space-y-3 mb-6">
              {items.map((item) => (
                <div key={(item._id || item.id)?.toString()} className="flex justify-between text-sm">
                  <span>
